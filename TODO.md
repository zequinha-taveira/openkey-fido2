...
- ✅ **Porte no_std do stack authenticator (pré-requisito do wiring)** — Desbloqueado e verificado em host: `ring` vendido em `examples/rp2350-firmware/vendor/ring` exclui fontes asm `armv4/armv7` que não montam em Thumb-2 (fallbacks C entram; ver `vendor/ring/build.rs:659-687`), `getrandom 0.2` com feature `custom` registrado em `examples/rp2350-firmware/src/main.rs:134-148` (semente `BootRandom` 128 bits do ROM + jitter + `splitmix64`; `ring::rand::SystemRandom` consome o mesmo getrandom; toolchain `gcc-arm-none-eabi` 10.3 via `[env] CC/AR/CXX_thumbv8m_main_none_eabihf`), APIs migradas para `Box<dyn core::error::Error>` (`firmware/authenticator/src/authenticator.rs:98-269`), `storage`/`ctap2`/`crypto` com `std` feature-gate e `extern crate alloc` nos testes; `cargo check -p authenticator --target thumbv8m.main-none-eabihf --no-default-features` e `cargo check` do `examples/rp2350-firmware` passam; `cargo test -p authenticator --no-default-features` (4 bootsel) e `--workspace` (38) passando; `FileStorageBackend`/`yubico_*` testes gated `#[cfg(all(test, feature="std"))]`. TRNG contínuo segue dev-only: nonces ECDSA ainda via PRNG semeado por boot — produção exigirá wiring do TRNG via PAC (follow-up).
...
- ✅ **`ykman list --serials` sobre PCSC (host-verificável)** — Driver CCID concreto já entregue: `examples/rp2350-firmware/src/composite.rs:71-111` expõe HID+CCID composto sobre `UsbBusAllocator` único, `firmware/transport/src/embedded/usb_ccid_backend.rs` responde como `CcidClass` com roteador ISO 7816 e `tools/hardware_check.py:128-212` valida via `winscard/ctypes` que o reader PCSC contém `"yubico yubikey"` no nome (VID:PID `1050:0407` → `Yubico YubiKey 5 0`, `ACTIVE_IDENTITY` em `composite.rs:57-64`, product `YubiKey 5` + manufacturer `Yubico`) e que `SELECT` OATH/Management retornam serial não-nulo (`sys:mgmt`); `ykman list` deriva PID via `_pid_from_name` quando o reader contém a substring (caso do flavor `yubikey5-identity`). Host-verificação via `just hardware-check`/`--json` (HID ping + ATR + SELECTs). Validação física `ykman list --serials` em placa ainda 🚧 (requer flash + PCSC).
...
- 🚧 **Validação física YubiKey** — `ykman list --serials` / Yubico Authenticator reconhecendo `Yubico YubiKey 5 0` (ou `4/5`) em `1050:0407` + `GetInfo.security` com `secure_boot=true` em placa; aguardando flash.
